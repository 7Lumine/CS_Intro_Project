...
def main():
    ...
    cap = cv.VideoCapture(CAMERA_INDEX) #カメラ開始
    if not cap.isOpened():
    ...
    if args.display_gui:
        ...
        if ret:
            gray_for_bg = cv.cvtColor(frame_for_bg, cv.COLOR_BGR2GRAY) # 計算量を抑えるためグレースケール変換
    ...
    try:
        while True:
            ...
            if avg_background is None and args.display_gui:
                ...
                cv.accumulateWeighted(gray, avg_background, ACCUMULATE_WEIGHT) # 背景画像を更新
                ...
                if motion_detection_enabled and motion_factor > MOTION_FACTOR_TH: # モーション検知したら
